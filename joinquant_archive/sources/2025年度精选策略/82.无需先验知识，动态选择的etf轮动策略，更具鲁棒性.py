# 克隆自聚宽文章：https://www.joinquant.com/post/24429
...
def get_signal(context):
    ...
    if g.use_dynamic_target_market:
        ...
        g.target_market = str(g.df.iloc[-1,0]) #不用最高的来处理
    ...
    #send_message("\n行情统计结果表:\n%s" % (g.df))
    if g.df.iloc[0,2] < 0.1 or g.df.iloc[0,3] < 0: #  or (g.df.iloc[0,2] + g.df.iloc[0,3] )> 25: # 均不符合买入条件
    ...
    if g.df.iloc[0,0] in ['510880.XSHG','510500.XSHG']:
        return 'sell_the_stocks' 
    '''
    if g.df.iloc[0,0] in ['510180.XSHG', '159901.XSHE']:
        g.df.iloc[0,0] = '150019.XSHE'
    '''
    # 表不为空,持有排名前三名的股
    if g.df.shape[0] > 22 :
        ...
        return 'BUY'
    # 表不为空,持有排名前两名的股
    if g.df.shape[0] > 21 :# 表不为空,持有排名前两名的股
        #g.buy.append(str(g.df.iloc[0,0]))
        g.buy.append(str(g.df.iloc[1,0]))
        ...
        return 'BUY'    
            ...
            #g.buy.append('159901.XSHE')
        #    return 'BUY'
        g.buy.append(str(g.df.iloc[0,0]))    
        #g.buy.append(g.ETFList[0])
        log.info("交易信号:持有 %s" % (g.buy))
        return 'BUY'
