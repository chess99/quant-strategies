...
MODEL_TO_EXPERIMENT = {
    ...
    "emerging-improvement": "h15",
    "emerging-momentum": "h16",
    "dual-track-growth-momentum": "h17",
    "quality-growth-momentum": "h18",
    "growth-acceleration-momentum": "h19",
}
...
def prepare_growth_rows(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    previous_profit = pd.to_numeric(
        grouped["quarter_parent_net_profit"].shift(1), errors="coerce"
    )
    previous_revenue = pd.to_numeric(
        grouped["quarter_revenue"].shift(1), errors="coerce"
    )
    ...
    result["previous_profit"] = previous_profit.where(exact_year)
    result["previous_revenue"] = previous_revenue.where(exact_year)
    ...
    turnaround_valid = (
        exact_year
        & previous_profit.le(0)
        & current_profit.gt(previous_profit)
        & revenue_valid
    )
    turnaround_denominator = current_profit.abs() + previous_profit.abs()
    result["turnaround_improvement"] = np.where(
        turnaround_valid & turnaround_denominator.gt(0),
        (current_profit - previous_profit) / turnaround_denominator,
        np.nan,
    )
    result["growth_track"] = pd.Series(pd.NA, index=result.index, dtype="string")
    result.loc[profit_valid & revenue_valid, "growth_track"] = "established"
    result.loc[turnaround_valid, "growth_track"] = "emerging"
    chronological = result.sort_values(["symbol", "report_date", "notice_date"]).copy()
    chronological["fiscal_period"] = (
        chronological["fiscal_year"] * 4 + chronological["fiscal_quarter"]
    )
    symbol_grouped = chronological.groupby("symbol", sort=False)
    previous_period = symbol_grouped["fiscal_period"].shift(1)
    previous_profit_growth = symbol_grouped["profit_growth"].shift(1)
    adjacent_quarter = previous_period.eq(chronological["fiscal_period"] - 1)
    chronological["profit_growth_acceleration"] = np.where(
        adjacent_quarter
        & chronological["profit_growth"].notna()
        & previous_profit_growth.notna(),
        chronological["profit_growth"] - previous_profit_growth,
        np.nan,
    )
    result["profit_growth_acceleration"] = chronological[
...
def latest_annual_quality_snapshot(
    prepared: pd.DataFrame, observation_date, maximum_age_days: int = 550
) -> pd.DataFrame:
    required = {"symbol", "report_date", "notice_date", "annual_roe"}
    missing = required.difference(prepared.columns)
    if missing:
        raise ValueError(f"quality rows are missing columns: {sorted(missing)}")
    date = pd.Timestamp(observation_date).normalize()
    visible = prepared[
        pd.to_datetime(prepared["notice_date"]).le(date)
        & pd.to_numeric(prepared["annual_roe"], errors="coerce").notna()
    ].copy()
    latest = (
        visible.sort_values(["symbol", "report_date", "notice_date"])
        .groupby("symbol", as_index=False)
    ...
    age = (date - pd.to_datetime(latest["report_date"])).dt.days
    result = latest.loc[age.le(maximum_age_days), ["symbol", "annual_roe"]].copy()
    return result.rename(columns={"annual_roe": "latest_annual_roe"}).reset_index(
        drop=True
    )
...
def select_candidates(
    ...
    growth_weight: float = 1.0 / 3.0,
    momentum_weight: float = 1.0 / 3.0,
    quality_weight: float = 1.0 / 3.0,
) -> pd.DataFrame:
    ...
    if "turnaround_improvement" not in frame:
        frame["turnaround_improvement"] = np.nan
    if "growth_track" not in frame:
        frame["growth_track"] = pd.Series(pd.NA, index=frame.index, dtype="string")
    if "profit_growth_acceleration" not in frame:
        frame["profit_growth_acceleration"] = np.nan
    if "latest_annual_roe" not in frame:
        frame["latest_annual_roe"] = np.nan
    frame["improvement_rank"] = _percentile(frame["turnaround_improvement"])
    frame["emerging_score"] = (
        frame["improvement_rank"] + frame["revenue_rank"]
    ) / 2.0
    frame["emerging_momentum_score"] = (
        frame["emerging_score"] + frame["momentum_score"]
    ) / 2.0
    frame["quality_score"] = _percentile(frame["latest_annual_roe"])
    quality_total_weight = growth_weight + momentum_weight + quality_weight
    if quality_total_weight <= 0:
        raise ValueError("quality model weights must sum to a positive value")
    frame["quality_growth_momentum_score"] = (
        frame["growth_score"] * growth_weight
        + frame["momentum_score"] * momentum_weight
        + frame["quality_score"] * quality_weight
    ) / quality_total_weight
    frame["acceleration_score"] = _percentile(frame["profit_growth_acceleration"])
    frame["accelerating_growth_score"] = (
        frame["profit_rank"] + frame["revenue_rank"] + frame["acceleration_score"]
    ) / 3.0
    frame["growth_acceleration_momentum_score"] = (
        frame["accelerating_growth_score"] + frame["momentum_score"]
    ) / 2.0
    ...
    elif model in ("huachuang-2-lite", "huachuang-2-risk-scaled"):
        eligible = frame[
            frame["growth_score"].ge(percentile_cutoff)
            & frame["momentum_score"].ge(percentile_cutoff)
        ]
        score = "combined_score"
    elif model == "emerging-improvement":
        eligible = frame[frame["growth_track"].eq("emerging")].dropna(
            subset=["emerging_score"]
        )
        score = "emerging_score"
    elif model == "emerging-momentum":
        eligible = frame[frame["growth_track"].eq("emerging")].dropna(
            subset=["emerging_momentum_score"]
        )
        score = "emerging_momentum_score"
    elif model == "quality-growth-momentum":
        eligible = frame[frame["growth_track"].eq("established")].dropna(
            subset=["quality_growth_momentum_score"]
        )
        score = "quality_growth_momentum_score"
    elif model == "growth-acceleration-momentum":
        eligible = frame[frame["growth_track"].eq("established")].dropna(
            subset=["growth_acceleration_momentum_score"]
        )
        score = "growth_acceleration_momentum_score"
    else:
        established = (
            frame[frame["growth_track"].eq("established")]
            .dropna(subset=["combined_score"])
            .sort_values(["combined_score", "symbol"], ascending=[False, True])
            .head(min(20, positions))
        )
        emerging_quota = max(0, positions - min(20, positions))
        emerging = (
            frame[frame["growth_track"].eq("emerging")]
            .dropna(subset=["emerging_momentum_score"])
            .sort_values(
                ["emerging_momentum_score", "symbol"], ascending=[False, True]
            )
            .head(emerging_quota)
        )
        return pd.concat([established, emerging], ignore_index=True)
    return (
...
def build_selections(
    ...
    models: tuple[str, ...],
    positions: int,
    quality_maximum_age_days: int = 550,
    ...
    for row in schedule.itertuples(index=False):
        ...
        quality = latest_annual_quality_snapshot(
            growth_rows,
            observation,
            maximum_age_days=quality_maximum_age_days,
        )
        features = price[price["symbol"].isin(active)].merge(
            ...
            on="symbol",
            how="inner",
        ).merge(quality, on="symbol", how="left")
        for model in models:
            options = (selection_options or {}).get(model, {})
            selected = select_candidates(
                features, model, positions=positions, **options
            )
