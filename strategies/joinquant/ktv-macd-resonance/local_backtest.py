...
class LocalBacktester:
    ...
    def _sell(self, symbol, trade_date, observation_date, decision):
    ...
    def run(self):
        ...
        for trade_date in trade_dates:
            ...
            if (
                ...
                for symbol in self.data.members_on(
                    ...
                    if window is None:
                        continue
                    signal = self.logic.entry_signal(window)
        ...
        if not trades.empty:
            trades.sort_values(["date", "symbol", "side"], inplace=True)
...
def build_report(result, run_id):
    ...
    if metrics["annualized_excess_return"] > 0 and metrics["sharpe"] > 0:
        ...
        inference = "当前基线尚未证明优于宽基指数，不应直接进入参数寻优。"
    return f"""# KTV + MACD 本地 Qlib 回测
