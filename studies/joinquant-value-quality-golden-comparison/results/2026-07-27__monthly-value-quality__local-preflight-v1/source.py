...
def load_or_build_snapshots(store, observation_dates, execution_dates):
    ...
    return valuation.frame, state.frame, fundamentals.frame, industry_frame, scans


def build_targets(store: ResearchDataStore, schedule: pd.DataFrame):
    valuation, state, fundamentals, industry, scans = load_or_build_snapshots(
        store,
    ...
    merged = merged.merge(
        master[["symbol", "exchange", "start_date", "end_date", "display_name"]],
        on="symbol",
        how="left",
...
def run(args) -> Path:
    ...
    checks = {
        ...
        "selected_state_quality_at_least_b": bool(
            state["status_quality"].isin(["A", "B"]).all()
            and state["st_quality"].isin(["A", "B"]).all()
            and state["limit_quality"].isin(["A", "B"]).all()
        ),
