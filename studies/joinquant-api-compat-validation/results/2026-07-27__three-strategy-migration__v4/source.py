...
DEFAULT_RUN_ID = "three-strategy-migration__v4"
...
def fundamental_quality_logic(api: JoinQuantCompat) -> pd.DataFrame:
    ...
    result = frame.sort_values("roe", ascending=False).reset_index(drop=True)
    result.attrs["strategy_provenance"] = {
        "fundamentals": fundamental_provenance,
        "market_state": api.last_query_provenance.copy(),
    }
    return result
...
def _versioned(provenance: dict) -> bool:
    return bool(
        provenance.get("data_version")
        or provenance.get("manifest_sha256")
        or (provenance.get("platform_audit") or {}).get("sha256")
    )
...
def main() -> int:
    args = parse_args()
    result = Path(__file__).parent / "results" / f"2026-07-27__{args.run_id}"
    if result.exists():
    ...
    fundamental_provenance = fundamentals.attrs["strategy_provenance"]
    ...
    checks = {
        ...
        "etf_four_ranked": bool(len(etf) == 4 and etf["momentum_20d"].notna().all()),
        ...
        "all_three_have_provenance": bool(
            timing_provenance.get("quality_grade")
            and etf_provenance.get("quality_grade")
            and fundamental_provenance["fundamentals"].get("quality_grade")
            and fundamental_provenance["market_state"].get("quality_grade")
        ),
        "all_three_have_versioned_provenance": bool(
            _versioned(timing_provenance)
            and _versioned(etf_provenance)
            and _versioned(fundamental_provenance["fundamentals"])
            and _versioned(fundamental_provenance["market_state"])
        ),
    }
