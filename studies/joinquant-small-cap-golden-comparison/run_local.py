...
def load_or_build_monthly_snapshots(store, observation_dates, execution_dates):
    ...
    if valuation_path.is_file() and state_path.is_file() and audit_path.is_file():
    ...
    return valuation_result.frame, state_result.frame, scans
...
def build_targets(store: ResearchDataStore, schedule: pd.DataFrame):
    ...
    valuation, state, scan_audits = load_or_build_monthly_snapshots(
        store, observation_dates, execution_dates
    )
