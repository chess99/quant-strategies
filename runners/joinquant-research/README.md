...
- 2026-08-12：在聚宽 Research Python 3 内核完成真实平台冒烟测试。
- 验证区间：2024-01-02 至 2024-03-29，共 58 个交易日。
- 已验证：模块导入、交易日历、真实行情读取、日频账本、指标计算、
  `manifest.json` / 报告 / CSV / ZIP 导出。
- 生成文件：`exports/platform-smoke-20240812-v3.zip`。
- 本次两 ETF 动量样例未产生交易，期末净值为 1,000,000；这证明执行和导出链路可用，
  但不作为撮合正确性的黄金对照。下一步仍需选择一个必然持仓的固定目标样例，核对成交、
  费用和持仓账本后再升格为默认研究后端。
