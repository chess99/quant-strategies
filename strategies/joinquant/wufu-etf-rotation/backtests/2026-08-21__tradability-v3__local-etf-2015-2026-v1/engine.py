...
V3_PROTOCOL_PATH = FAMILY / "protocols" / "2026-08-21-wufu-tradability-v3.json"
...
class StrategyConfig:
    ...
    ordinal_rank_buffer_extra: int = 0
    ...
    regime_use_hysteresis: bool = True
...
def select_with_rank_buffer(
    ...
    """保留仍处于 Top(K+extra) 的老持仓，再按当前名次补足 TopK。"""

    if not ranked or top_k <= 0:
        return []
    ranked = sorted(ranked, key=lambda item: (-item[1], item[0]))
    width = min(len(ranked), top_k + max(0, int(extra_ranks)))
    rank_map = {symbol: index for index, (symbol, _) in enumerate(ranked)}
    eligible = {symbol for symbol, _ in ranked[:width]}
    retained = [symbol for symbol in current_holdings if symbol in eligible]
    retained.sort(key=lambda symbol: rank_map[symbol])
    result = retained[:top_k]
    for symbol, _ in ranked:
        if len(result) >= top_k:
            break
        if symbol not in result:
            result.append(symbol)
    return sorted(result, key=lambda symbol: rank_map[symbol])


def is_scheduled_rebalance(
    calendar_position: int,
    interval_days: int,
    phase: int = 0,
) -> bool:
    """按全样本交易日位置判断调仓日，保证分段回测的日程相位一致。"""

    interval = int(interval_days)
    selected_phase = int(phase)
    if interval <= 0:
        raise ValueError("rebalance interval must be positive")
    if selected_phase < 0 or selected_phase >= interval:
        raise ValueError("rebalance phase must satisfy 0 <= phase < interval")
    return calendar_position >= selected_phase and (
        calendar_position - selected_phase
    ) % interval == 0
...
def _direct_weak_array(index_close: np.ndarray, ma_days: int = 10) -> np.ndarray:
    """逐日计算直接 3/4 弱市投票，不继承原 A4 的迟滞状态。"""

    values = np.asarray(index_close, dtype=float)
    result = np.zeros(len(values), dtype=bool)
    for row in range(ma_days - 1, len(values)):
        current = values[row]
        moving_average = np.nanmean(values[row - ma_days + 1 : row + 1], axis=0)
        valid = np.isfinite(current) & np.isfinite(moving_average)
        result[row] = int(np.sum(valid & (current < moving_average))) >= 3
    return result
...
def load_market_data(
    ...
    *,
    ...
    if index_frame is None:
        ...
        canonical = index_frame.sort_index().reset_index(names="date")
        canonical_payload = canonical.to_csv(index=False).encode("utf-8")
        index_audit = {
            "provider": "caller-supplied-frozen-csv",
            "rows": int(len(index_frame)),
            "csv_sha256": hashlib.sha256(canonical_payload).hexdigest(),
        }
    ...
    weak_vote_state = _direct_weak_array(index_close)
    ...
    return MarketData(
        ...
        weak_vote_state=weak_vote_state,
...
def tradability_v3_primary_config(**changes: Any) -> StrategyConfig:
    """返回 2026-08-21 事前冻结的可交易化 v3 主模型。"""

    config = StrategyConfig(
        name="T3_primary",
        top_k=3,
        universe="original_like",
        lookback=25,
        r2_threshold=0.4,
        score_max=5.0,
        buffer_ratio=0.9,
        ordinal_rank_buffer_extra=2,
        ...
        use_buffer=True,
        use_regime=True,
        regime_use_hysteresis=False,
        use_mainline=False,
        use_retention=False,
        regime_pool_switch=True,
        regime_filter_relaxation=False,
        regime_dynamic_lookback=False,
        regime_buffer_disable=False,
        single_side_total_cost_bp=10.0,
        initial_cash=200_000.0,
    )
    return replace(config, **changes) if changes else config


def tradability_v3_ordered_configs() -> list[StrategyConfig]:
    """返回事前冻结的 T0 -> T3 有序消融。"""

    daily = tradability_v3_primary_config(
        name="T0",
        ordinal_rank_buffer_extra=0,
        rebalance_interval_days=1,
        use_regime=False,
    )
    return [
        daily,
        replace(daily, name="T1", use_regime=True),
        replace(daily, name="T2", use_regime=True, ordinal_rank_buffer_extra=2),
        tradability_v3_primary_config(),
    ]


def tradability_v3_structural_configs() -> list[StrategyConfig]:
    """返回协议锁定的 96 组结构压力矩阵。"""

    rows = []
    for top_k, use_r2, use_regime, interval, extra in itertools.product(
        (1, 3),
        (False, True),
        (False, True),
        (1, 5, 10, 20),
        ...
        name = (
            f"S_top{top_k}_r2{int(use_r2)}_weak{int(use_regime)}_"
            f"d{interval}_rank{extra}"
        )
        rows.append(
            tradability_v3_primary_config(
                name=name,
                ...
                ordinal_rank_buffer_extra=extra,
            )
        )
    return rows


def _weak_signal(data: MarketData, row: int, config: StrategyConfig) -> bool:
    states = data.weak_state if config.regime_use_hysteresis else data.weak_vote_state
    return bool(states[row])
...
def _select_targets(
    ...
    weak = bool(config.use_regime and _weak_signal(data, row, config))
    signal_lookback = (
    ...
    if (
        config.use_buffer
        and not disable_buffer
        and config.ordinal_rank_buffer_extra > 0
    ):
        selected = select_with_rank_buffer(
            ranked,
            holdings,
            config.top_k,
            config.ordinal_rank_buffer_extra,
        )
    else:
...
def run_simulation(
    ...
    *,
    ...
    schedule_anchor = int(np.searchsorted(data.dates, START_DATE))
    # Validate once even when the requested period happens to contain no phase hit.
    is_scheduled_rebalance(0, config.rebalance_interval_days, config.rebalance_phase)
    ...
    for row in output_rows:
        ...
        calendar_position = int(row - schedule_anchor)
        scheduled_rebalance = is_scheduled_rebalance(
            calendar_position,
            config.rebalance_interval_days,
            config.rebalance_phase,
        )
        if observation_row >= 0 and scheduled_rebalance:
            targets, diagnostics = _select_targets(
                observation_row, holdings_for_signal, data, cache, config
            )
        else:
            retained_targets = sorted(
                symbol for symbol, shares in positions.items() if shares > 0
            )
            targets = retained_targets or [CASH_ETF]
            diagnostics = {
                "weak": _weak_signal(data, observation_row, config)
                if observation_row >= 0
                else False,
                "choppy": bool(data.choppy_state[observation_row])
                if observation_row >= 0
                else False,
            }
        diagnostics["scheduled_rebalance"] = scheduled_rebalance
        ...
        if scheduled_rebalance and config.adv_participation is None:
...
def _result_row(result: SimulationResult) -> dict[str, Any]:
    ...
    return {
        ...
        "ordinal_rank_buffer_extra": config.ordinal_rank_buffer_extra,
        "rebalance_interval_days": config.rebalance_interval_days,
        "rebalance_phase": config.rebalance_phase,
        "regime_use_hysteresis": config.regime_use_hysteresis,
...
def _tradability_v3_result_row(result: SimulationResult) -> dict[str, Any]:
    config = result.config
    return {
        **_result_row(result),
        "use_r2": config.use_r2,
        ...
        "use_buffer": config.use_buffer,
        "use_regime": config.use_regime,
        "weak_pool_switch": bool(config.use_regime and config.regime_pool_switch),
        "use_mainline": config.use_mainline,
        "use_retention": config.use_retention,
        "regime_filter_relaxation": config.regime_filter_relaxation,
        "regime_dynamic_lookback": config.regime_dynamic_lookback,
        "regime_buffer_disable": config.regime_buffer_disable,
    }


def _validate_tradability_v3_inputs(data: MarketData) -> dict[str, Any]:
    protocol = json.loads(V3_PROTOCOL_PATH.read_text(encoding="utf-8"))
    expected = protocol["frozen_inputs"]["data_manifest_hashes"]
    actual = {
        **data.input_hashes,
        "four_index_csv": data.reference["index_audit"]["csv_sha256"],
    }
    mismatches = {
    ...
    parent_manifest = FAMILY / protocol["hypothesis_origin"]["parent_archive"] / "manifest.json"
    parent_expected = protocol["hypothesis_origin"]["parent_manifest_sha256"]
    parent_actual = file_sha256(parent_manifest)
    ...
    if mismatches:
        raise ValueError(f"tradability v3 frozen-input mismatch: {mismatches}")
    return {
        "protocol_id": protocol["protocol_id"],
        "protocol_sha256": file_sha256(V3_PROTOCOL_PATH),
        "frozen_before_v3_results": protocol["frozen_before_v3_results"],
        "input_hashes": actual,
...
def _tradability_v3_gates(
    primary: SimulationResult,
    structural: pd.DataFrame,
    schedule_phase: pd.DataFrame,
    ...
    pbo: dict[str, Any],
    dsr: dict[str, Any],
    a6_top3: dict[str, Any],
) -> pd.DataFrame:
    criteria = json.loads(V3_PROTOCOL_PATH.read_text(encoding="utf-8"))["success_criteria"]
    primary_metrics = primary.metrics
    cost20 = cost_stress.loc[cost_stress["cost_bp"].eq(20.0)].iloc[0]
    capacity10m = capacity_stress.loc[
        capacity_stress["initial_cash"].eq(10_000_000.0)
        & capacity_stress["adv_participation"].eq(0.005)
    ].iloc[0]
    turnover_reduction = 1.0 - (
        primary_metrics["annualized_turnover"] / a6_top3["annualized_turnover"]
    )
    sharpe_loss = a6_top3["sharpe"] - primary_metrics["sharpe"]
    observed = {
        "primary_annualized_turnover_reduction_vs_A6_top3": turnover_reduction,
        "primary_max_sharpe_loss_vs_A6_top3_at_2bp": sharpe_loss,
        "primary_10bp_min_cagr": primary_metrics["annualized_return"],
        "primary_10bp_min_sharpe": primary_metrics["sharpe"],
        "primary_20bp_min_cagr": float(cost20["annualized_return"]),
        "primary_max_drawdown": primary_metrics["maximum_drawdown"],
        "primary_worst_rolling_three_year_min": primary_metrics[
            "worst_rolling_three_year_return"
        ],
        "capacity_10m_0_5pct_adv_min_average_exposure": float(
            capacity10m["average_exposure"]
        ),
        "exclusion_min_cagr_retention": float(exclusion_stress["cagr_retention"].min()),
        "structural_grid_10bp_positive_ratio": float(
            structural["annualized_return"].gt(0.0).mean()
        ),
        "structural_grid_10bp_median_sharpe": float(structural["sharpe"].median()),
        "schedule_phase_positive_ratio": float(
            schedule_phase["annualized_return"].gt(0.0).mean()
        ),
        "walk_forward_min_cagr": walk_forward_summary["annualized_return"],
        "walk_forward_worst_year": walk_forward_summary["worst_year"],
        "pbo_max": pbo["pbo"],
        "dsr_probability_min": dsr["deflated_sharpe_probability"],
    }
    maximum_ids = {
        "primary_max_sharpe_loss_vs_A6_top3_at_2bp",
    ...
    for criterion, threshold in criteria.items():
        ...
        operator = "<=" if criterion in maximum_ids else ">="
...
def run_tradability_v3_suite(data: MarketData) -> dict[str, Any]:
    """运行 2026-08-21 事前冻结的可交易化 v3 完整矩阵。"""

    input_validation = _validate_tradability_v3_inputs(data)
    cache = FeatureCache(data)

    print("[1/7] T0-T3 ordered ablation", flush=True)
    ordered_results = {}
    ordered_rows = []
    for config in tradability_v3_ordered_configs():
        result = run_simulation(
            data,
            cache,
            config,
            capture_details=config.name == "T3_primary",
        )
        ordered_results[config.name] = result
        ordered_rows.append(_tradability_v3_result_row(result))
    primary = ordered_results["T3_primary"]

    print("[2/7] frozen 96-trial structural grid", flush=True)
    structural_configs = tradability_v3_structural_configs()
    structural_rows = []
    structural_returns = []
    for index, config in enumerate(structural_configs, start=1):
        result = run_simulation(data, cache, config)
        structural_rows.append(_tradability_v3_result_row(result))
        structural_returns.append(result.equity["daily_return"].to_numpy(dtype=np.float32))
        if index % 24 == 0:
            print(f"  structural trials {index}/96", flush=True)
    structural = pd.DataFrame(structural_rows)
    return_matrix = np.vstack(structural_returns)
    result_dates = pd.DatetimeIndex(primary.equity["trade_date"])

    print("[3/7] schedule phase, lookback, and universe pressure", flush=True)
    phase_rows = []
    for phase in range(5):
        result = run_simulation(
            data,
            cache,
            tradability_v3_primary_config(name=f"phase_{phase}", rebalance_phase=phase),
        )
        phase_rows.append(_tradability_v3_result_row(result))
    lookback_rows = []
    for lookback in (20, 25, 30):
        result = run_simulation(
            data,
            cache,
            tradability_v3_primary_config(name=f"lookback_{lookback}", lookback=lookback),
        )
        lookback_rows.append(_tradability_v3_result_row(result))
    universe_rows = []
    for universe in (
        "fixed_only",
        "original_like",
        ...
        result = run_simulation(
            data,
            cache,
            tradability_v3_primary_config(name=f"universe_{universe}", universe=universe),
        )
        universe_rows.append(_tradability_v3_result_row(result))

    print("[4/7] cost and ADV-capacity pressure", flush=True)
    cost_rows = []
    for cost in (2.0, 5.0, 10.0, 15.0, 20.0, 30.0):
        result = run_simulation(
            data,
            cache,
            tradability_v3_primary_config(
                name=f"cost_{cost:g}bp", single_side_total_cost_bp=cost
            ),
        )
        cost_rows.append(_tradability_v3_result_row(result))
    capacity_rows = []
    for capital, participation in itertools.product(
        (200_000.0, 1_000_000.0, 5_000_000.0, 10_000_000.0, 50_000_000.0),
        (0.005, 0.01, 0.05),
    ):
        result = run_simulation(
            ...
            cache,
            tradability_v3_primary_config(
                name=f"capacity_{capital:g}_{participation:g}",
                initial_cash=capital,
                adv_participation=participation,
            ),
        )
        capacity_rows.append(_tradability_v3_result_row(result))

    print("[5/7] concentration and contributor exclusions", flush=True)
    asset_contributions, concentration = contribution_concentration(primary)
    ranked_symbols = [
        symbol for symbol in asset_contributions["symbol"].tolist() if symbol != CASH_ETF
    ]
    exclusion_rows = []
    for count in (1, 3, 5):
        excluded = tuple(ranked_symbols[:count])
        result = run_simulation(
            ...
            cache,
            tradability_v3_primary_config(
                name=f"exclude_top{count}_contributors",
                excluded_symbols=excluded,
            ),
        )
        row = _tradability_v3_result_row(result)
        row["excluded_symbols"] = ";".join(excluded)
        row["cagr_retention"] = (
            row["annualized_return"] / primary.metrics["annualized_return"]
            if primary.metrics["annualized_return"] != 0.0
            else math.nan
        )
        exclusion_rows.append(row)

    print("[6/7] time stability, PBO, DSR, and expanding-window", flush=True)
    pbo_splits, pbo = compute_pbo(
        return_matrix,
        result_dates,
        [config.name for config in structural_configs],
    )
    dsr = deflated_sharpe_probability(
        primary.equity["daily_return"].to_numpy(dtype=float),
        return_matrix,
    )
    walk_forward, walk_equity, walk_forward_summary = expanding_walk_forward(
        return_matrix,
        result_dates,
        [config.name for config in structural_configs],
    )
    yearly = yearly_metrics(primary.equity)
    periods = period_metrics(primary.equity)
    regimes = regime_metrics(primary, data)
    rolling = rolling_three_year_metrics(primary.equity)

    print("[7/7] preregistered success gates", flush=True)
    ...
    a6_top3 = parent_manifest["metrics"]["A6_top3"]
    ...
    capacity_stress = pd.DataFrame(capacity_rows)
    exclusion_stress = pd.DataFrame(exclusion_rows)
    success_gates = _tradability_v3_gates(
        primary,
        structural,
        phase,
        cost_stress,
    ...
    experiment_counts = {
        "ordered_ablation": len(ordered_rows),
        "structural_grid": len(structural_rows),
        "schedule_phase": len(phase_rows),
        "lookback_stress": len(lookback_rows),
        "universe_stress": len(universe_rows),
        "cost_stress": len(cost_rows),
        "capacity_stress": len(capacity_rows),
        "contributor_exclusion": len(exclusion_rows),
        ...
        "pbo_splits": int(len(pbo_splits)),
        "walk_forward_years": int(len(walk_forward)),
        "success_gates": int(len(success_gates)),
        "passed_success_gates": int(success_gates["passed"].sum()),
    }
    return {
        "primary": primary,
        "ordered_ablation": ordered,
        "structural_grid": structural,
        "schedule_phase": phase,
        "lookback_stress": lookback,
        "universe_stress": universe,
...
def build_tradability_v3_report(bundle: dict[str, Any], data: MarketData) -> str:
    primary = bundle["primary"].metrics
    gates = bundle["success_gates"]
    passed = int(gates["passed"].sum())
    total = int(len(gates))
    overall = "未通过完整事前成功门槛" if passed < total else "通过完整事前成功门槛"
    ordered = bundle["ordered_ablation"].copy()
    structural = bundle["structural_grid"]
    phase = bundle["schedule_phase"]
    cost20 = bundle["cost_stress"].loc[bundle["cost_stress"]["cost_bp"].eq(20.0)].iloc[0]
    capacity10m = bundle["capacity_stress"].loc[
        bundle["capacity_stress"]["initial_cash"].eq(10_000_000.0)
        & bundle["capacity_stress"]["adv_participation"].eq(0.005)
    ].iloc[0]
    a6 = bundle["a6_top3"]
    turnover_reduction = 1.0 - primary["annualized_turnover"] / a6["annualized_turnover"]
    best_year = bundle["yearly"].sort_values("annualized_return", ascending=False).iloc[0]
    worst_year = bundle["yearly"].sort_values("annualized_return").iloc[0]
    gate_table = gates.copy()
    gate_table["passed"] = gate_table["passed"].map({True: "通过", False: "未通过"})
    direct_count = bundle["experiment_counts"]["direct_backtests_total"]
    structural_positive = float(structural["annualized_return"].gt(0.0).mean())
    phase_positive = float(phase["annualized_return"].gt(0.0).mean())
    return f"""# 五福可交易化 v3 事前实验报告

## 结论

主模型 T3 的最终判定是：**{overall}**。16 项冻结门槛通过 {passed} 项、未通过
{total - passed} 项。本轮共运行 {direct_count} 次直接回测，包含全部失败组合；没有用结构矩阵中的
历史冠军替换事前指定的 T3。

T3 在单边总成本 10bp 下年化 {_pct(primary['annualized_return'])}、Sharpe
{_num(primary['sharpe'])}、最大回撤 {_pct(primary['maximum_drawdown'])}、年化换手
{_num(primary['annualized_turnover'], 1)} 倍。相对旧 A6 Top3 的年化换手
{_num(a6['annualized_turnover'], 1)} 倍，降幅 {_pct(turnover_reduction)}。这回答了本轮最核心的
“能否把五福结构压到更可交易的频率”问题，但是否值得进入平台完整仿真必须服从下方全部门槛，而
不能只看换手下降。

## 事实：有序消融

{_markdown_table(ordered, ['trial', 'annualized_return', 'sharpe', 'maximum_drawdown', 'annualized_turnover', 'worst_rolling_three_year_return'])}
...
- 单边 20bp 时，T3 年化 {_pct(float(cost20['annualized_return']))}、Sharpe
  {_num(float(cost20['sharpe']))}；
- 1000 万元、0.5% ADV 场景的平均风险暴露为
  {_pct(float(capacity10m['average_exposure']))}；
- 删除事后贡献最高的 1/3/5 只 ETF 后，最低 CAGR 保留比例为
  {_pct(float(bundle['exclusion_stress']['cagr_retention'].min()))}；
- 正收益贡献 Top1/Top3/Top5 占比分别为
  {_pct(bundle['concentration']['top1_positive_contribution_share'])}/
  {_pct(bundle['concentration']['top3_positive_contribution_share'])}/
  {_pct(bundle['concentration']['top5_positive_contribution_share'])}。
...
- 96 组结构矩阵在 10bp 下正年化比例 {_pct(structural_positive)}，中位 Sharpe
  {_num(float(structural['sharpe'].median()))}；
- 五个周频相位中正年化比例 {_pct(phase_positive)}，年化范围
  {_pct(float(phase['annualized_return'].min()))}—{_pct(float(phase['annualized_return'].max()))}；
- CSCV/PBO 为 {_pct(bundle['pbo']['pbo'])}，T3 的 DSR 概率为
  {_pct(bundle['dsr']['deflated_sharpe_probability'])}；
- 2019 年起 expanding-window 年化
  {_pct(bundle['walk_forward_summary']['annualized_return'])}，最差年度
  {_pct(bundle['walk_forward_summary']['worst_year'])}；
- 最好年度为 {int(best_year['year'])} 年 {_pct(float(best_year['annualized_return']))}，最差年度为
  {int(worst_year['year'])} 年 {_pct(float(worst_year['annualized_return']))}；最差滚动三年收益为
  {_pct(primary['worst_rolling_three_year_return'])}。

## 冻结成功门槛

{_markdown_table(gate_table, ['criterion', 'observed', 'operator', 'threshold', 'passed'])}
...
输入共 {len(data.dates)} 个含预热交易日、{len(data.symbols)} 只 B 级 ETF。所有 v3 结果均在协议
`2026-08-21-wufu-tradability-v3` 冻结之后生成。
"""


def create_tradability_v3_charts(output: Path, bundle: dict[str, Any]) -> None:
    assets = output / "assets"
    assets.mkdir(parents=True, exist_ok=True)
    figure, axes = plt.subplots(2, 2, figsize=(13, 8))
    primary = bundle["primary"]
    curve = primary.equity["total_value"] / primary.config.initial_cash
    axes[0, 0].plot(pd.to_datetime(primary.equity["trade_date"]), curve, color="#1f77b4")
    axes[0, 0].set_title("T3 primary net value (10bp)")
    ordered = bundle["ordered_ablation"]
    axes[0, 1].bar(ordered["trial"], ordered["annualized_return"], color="#4c78a8")
    axes[0, 1].axhline(0.0, color="black", linewidth=0.8)
    axes[0, 1].set_title("Ordered ablation CAGR")
    phase = bundle["schedule_phase"]
    axes[1, 0].bar(phase["rebalance_phase"].astype(str), phase["annualized_return"], color="#59a14f")
    axes[1, 0].axhline(0.0, color="black", linewidth=0.8)
    axes[1, 0].set_title("Five-day schedule phases")
    costs = bundle["cost_stress"]
    axes[1, 1].plot(costs["cost_bp"], costs["annualized_return"], marker="o", color="#e15759")
    axes[1, 1].axhline(0.0, color="black", linewidth=0.8)
    axes[1, 1].set_title("Cost stress CAGR")
    axes[1, 1].set_xlabel("single-side total cost (bp)")
    for axis in axes.flat:
        axis.grid(alpha=0.2)
    figure.tight_layout()
    figure.savefig(assets / "tradability-v3-summary.png", dpi=160)
    plt.close(figure)
...
def write_tradability_v3_archive(
    data: MarketData,
    bundle: dict[str, Any],
    *,
    run_name: str = "2026-08-21__tradability-v3__local-etf-2015-2026-v1",
) -> Path:
    output = FAMILY / "backtests" / run_name
    if output.exists():
        raise FileExistsError(f"immutable archive already exists: {output}")
    ...
    raw.mkdir(parents=True)
    csv_artifacts = {
        "ordered-ablation.csv": bundle["ordered_ablation"],
        "structural-grid.csv": bundle["structural_grid"],
        "schedule-phase.csv": bundle["schedule_phase"],
        "lookback-stress.csv": bundle["lookback_stress"],
        "universe-stress.csv": bundle["universe_stress"],
        "cost-stress.csv": bundle["cost_stress"],
        "capacity-stress.csv": bundle["capacity_stress"],
        "asset-contributions.csv": bundle["asset_contributions"],
        "contributor-exclusion.csv": bundle["exclusion_stress"],
        "yearly.csv": bundle["yearly"],
        "periods.csv": bundle["periods"],
        "regimes.csv": bundle["regimes"],
        "rolling-three-year.csv": bundle["rolling_three_year"],
        ...
        "walk-forward.csv": bundle["walk_forward"],
        "walk-forward-equity.csv": bundle["walk_forward_equity"],
        "success-gates.csv": bundle["success_gates"],
        "primary-equity.csv": bundle["primary"].equity,
        "primary-trades.csv": bundle["primary"].trades,
        "primary-positions.csv": bundle["primary"].positions,
        "primary-decisions.csv": bundle["primary"].decisions,
        "primary-contributions.csv": bundle["primary"].contributions,
        "index-close.csv": data.reference["index_raw"],
    }
    for name, frame in csv_artifacts.items():
        frame.to_csv(raw / name, index=False, encoding="utf-8")
    ...
    _write_json(raw / "walk-forward-summary.json", bundle["walk_forward_summary"])
    _write_json(raw / "concentration.json", bundle["concentration"])
    _write_json(raw / "input-validation.json", bundle["input_validation"])
    _write_json(raw / "experiment-counts.json", bundle["experiment_counts"])
    _write_json(
        output / "config.json",
        {
            "primary": asdict(tradability_v3_primary_config()),
            "protocol": json.loads(V3_PROTOCOL_PATH.read_text(encoding="utf-8")),
            "experiment_counts": bundle["experiment_counts"],
        },
    )
    ...
    _write_json(
        output / "audit.json",
        {
            ...
            "calendar_rows_with_warmup": int(len(data.dates)),
            "b_grade_etf_symbols": int(len(data.symbols)),
            "historically_terminated_etfs": int(
                data.master["lifecycle_status"].eq("delisted").sum()
            ),
            "input_validation": bundle["input_validation"],
            "index_audit": data.reference["index_audit"],
            "universe_audit": data.reference["universe_audit"],
            "causality": "observation close and earlier -> next trading day adjusted open",
            "schedule": (
                "full-sample trading-day anchor at 2015-01-01; no trades or partial-fill "
                "continuation on non-rebalance dates"
            ),
            "weak_regime": "direct daily 3-of-4 below causal MA10; no A4 hysteresis",
            "survivorship": (
                "daily listing/delisting lifecycle filter; historical delisted ETFs retained"
            ),
            "adjustment": "continuous adjusted open/close; raw RMB amount",
            "known_non_pit_fields": ["etf_category", "tracking_target", "display_name"],
            "platform_source_note": (
                "source.py is the JoinQuant fixed-pool transfer work package; engine.py is the "
                "exact local original_like proxy used for archived metrics"
            ),
        },
    )
    create_tradability_v3_charts(output, bundle)
    (output / "report.md").write_text(
        build_tradability_v3_report(bundle, data), encoding="utf-8"
    )
    source_hash = file_sha256(output / "source.py")
    engine_hash = file_sha256(output / "engine.py")
    artifact_hashes = {}
    ...
    manifest = {
        ...
        "source_file": "source.py",
        "platform": "joinquant",
        "execution_environment": "local-etf-daily",
        "strategy_id": "wufu-etf-rotation",
        "variant": "tradability-v3",
        "run_id": run_name,
        "period": {"start": str(START_DATE.date()), "end": str(END_DATE.date())},
        "benchmark": BENCHMARK,
        "cost": "single-side total 10bp primary; 2/5/10/15/20/30bp stress",
        "metrics": {
            "T3_primary": bundle["primary"].metrics,
            "A6_top3_reference": bundle["a6_top3"],
            "success_gates": {
                "passed": int(bundle["success_gates"]["passed"].sum()),
                "total": int(len(bundle["success_gates"])),
                "complete_protocol_pass": bool(bundle["success_gates"]["passed"].all()),
            },
        },
        "source_sha256": source_hash,
        "engine_sha256": engine_hash,
        "protocol_sha256": file_sha256(V3_PROTOCOL_PATH),
        "parent_manifest_sha256": bundle["input_validation"]["parent_manifest_sha256"],
        "experiment_counts": bundle["experiment_counts"],
        "artifacts": artifact_hashes,
    }
    _write_json(output / "manifest.json", manifest)
...
def main() -> None:
    ...
    parser.add_argument(
        "--suite",
        choices=("direct-decomposition", "tradability-v3"),
        default="direct-decomposition",
    )
    parser.add_argument("--run-name")
    arguments = parser.parse_args()
    if arguments.suite == "tradability-v3":
        frozen_index_path = (
            FAMILY
            / "backtests"
            / "2026-08-16__direct-decomposition__local-etf-2015-2026-v1"
        ...
        frozen_index = pd.read_csv(frozen_index_path)
        frozen_index["trade_date"] = pd.to_datetime(frozen_index["trade_date"])
        frozen_index = frozen_index.set_index("trade_date")
        data = load_market_data(arguments.data_root, index_frame=frozen_index)
        bundle = run_tradability_v3_suite(data)
        if arguments.archive:
            output = write_tradability_v3_archive(
                data,
                bundle,
                run_name=(
                    arguments.run_name
                    or "2026-08-21__tradability-v3__local-etf-2015-2026-v1"
                ),
            )
            print(output)
        else:
            print(bundle["success_gates"].to_string(index=False))
    else:
        ...
        if arguments.archive:
            output = write_archive(
                ...
                run_name=(
                    arguments.run_name
                    or "2026-08-16__direct-decomposition__local-etf-2015-2026-v1"
                ),
            )
            print(output)
        else:
