...
def load_external_targets(path: Path, schedule: pd.DataFrame) -> dict:
    frame = pd.read_csv(path)
    required = {"execution_date", "symbol"}
    missing = required.difference(frame.columns)
    if missing:
        raise ValueError(f"external targets are missing columns: {sorted(missing)}")
    ...
    sort_columns = ["execution_date"] + (["rank"] if "rank" in frame else [])
    frame = frame.sort_values(sort_columns, kind="stable")
    expected_dates = list(pd.to_datetime(schedule["execution_date"]).dt.normalize())
    observed_dates = list(frame["execution_date"].drop_duplicates())
    if set(observed_dates) != set(expected_dates):
        raise ValueError(
            "external target rebalance dates do not match schedule: "
            f"expected={len(expected_dates)}, observed={len(observed_dates)}"
        )
    targets = {}
    for date in expected_dates:
        symbols = frame.loc[frame["execution_date"].eq(date), "symbol"].tolist()
        if len(symbols) != HOLDINGS or len(set(symbols)) != HOLDINGS:
            raise ValueError(
                f"external targets for {date:%Y-%m-%d} must contain "
                f"{HOLDINGS} unique symbols"
            )
        targets[date] = symbols
    return targets
...
def score_value_quality(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    data = data[
        ...
        & data["quarter_roe"].between(0.0, 100.0)
        & data["quarter_roa"].between(0.0, 100.0)
        & data["quarter_gross_margin"].between(-100.0, 100.0)
        & data["quarter_net_margin"].between(0.0, 100.0)
        & data["total_assets"].gt(0.0)
        & data["debt_to_assets"].between(0.0, 1.5)
        & data["quarter_operating_cash_flow"].gt(0.0)
...
def run(args) -> Path:
    ...
    target_provenance = {"mode": "local-point-in-time-ranking"}
    if args.targets_csv is not None:
        target_path = Path(args.targets_csv).resolve()
        targets = load_external_targets(target_path, schedule)
        target_provenance = {
            "mode": "external-target-replay",
    ...
    if args.targets_csv is not None:
        outputs["target-override.csv"] = pd.DataFrame(
            [
                {
                    "execution_date": date,
                    "symbol": symbol,
                ...
                for date, symbols in targets.items()
