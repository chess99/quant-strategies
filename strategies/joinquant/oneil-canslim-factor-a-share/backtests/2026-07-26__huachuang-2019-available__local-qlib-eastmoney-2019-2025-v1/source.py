...
def latest_quarter_growths(quarterly):
    ...
    prior = prior.iloc[-1]
    mappings = (
        ("basic_eps", "eps_growth"),
        ("np_parent_company_owners", "profit_growth"),
        ("total_operating_revenue", "revenue_growth"),
    )
    for column, output in mappings:
...
def market_exposure(benchmark_closes, model):
    ...
    if model not in {"shenwan-2018-lite", "a-share-adaptive"}:
        return 1.0
