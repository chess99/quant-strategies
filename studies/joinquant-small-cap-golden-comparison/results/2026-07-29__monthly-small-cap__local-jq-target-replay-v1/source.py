...
def load_external_targets(path: Path, schedule: pd.DataFrame) -> dict:
    ...
    for date in expected_dates:
        ...
        if len(symbols) != STOCK_COUNT or len(set(symbols)) != STOCK_COUNT:
            raise ValueError(
                f"external targets for {date:%Y-%m-%d} must contain "
                f"{STOCK_COUNT} unique symbols"
            )
...
def run(args) -> Path:
    ...
    checks = {
        ...
        == 6115,
        "all_rebalances_have_ten_targets": len(targets) == len(schedule)
        and all(len(symbols) == STOCK_COUNT for symbols in targets.values()),
    ...
    manifest = {
        ...
        "strategy": {
            ...
            "ranking": (
                "daily_valuation.market_cap ascending"
                if args.targets_csv is None
                else "external target replay"
            ),
