...
def get_previous_closes(codes, previous_date):
    """批量取得上一交易日收盘价并转换为代码到价格的映射。"""
    if not codes:
        return {}
...
def filter_stocks(codes, current_data, security_frame, buy_date):
    ...
    for code in codes:
        ...
        if (buy_date - start_date).days < 365:
            skipped_recent += 1
            continue
    ...
    log.info(
        "股票过滤：输入=%d，可买=%d，北交所/无证券信息=%d，上市不足一年=%d，停牌/ST/退市=%d，价格无效/涨停=%d",
        len(codes),
        len(eligible),
        skipped_market,
        skipped_recent,
...
def calculate_target_stocks(context):
    ...
    closes = get_previous_closes(eligible, context.previous_date)
    targets = rank_holdings(
        holdings,
        closes,
        eligible,
        getattr(g, "stock_count", 100),
    )
