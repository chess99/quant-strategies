...
def calibrate_event(event):
    ...
    for clock in CHECK_TIMES:
        ...
        # JoinQuant Research currently ships an older pandas where
        # Series.to_numpy() is unavailable. ``values`` is equivalent here and
        # keeps this platform script compatible with both old and new pandas.
        history = entry.loc[entry["time"] <= cutoff, "close"].tail(30).values
