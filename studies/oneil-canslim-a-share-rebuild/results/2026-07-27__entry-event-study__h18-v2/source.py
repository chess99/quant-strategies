...
def compute_signal_frame(prices: pd.DataFrame) -> pd.DataFrame:
    ...
    volatility_20 = returns.shift(1).rolling(20, min_periods=20).std()
    volatility_60 = returns.shift(1).rolling(60, min_periods=50).std()
    volume_20 = volume.shift(1).rolling(20, min_periods=20).mean()
    volume_60 = volume.shift(1).rolling(60, min_periods=50).mean()
    frame["vcp_breakout"] = (
        frame["new_high_55"]
        & volatility_20.le(volatility_60 * 0.75)
        & volume_20.le(volume_60 * 0.80)
    )
    structured = pd.Series(False, index=frame.index)
    for window in (30, 45, 65, 90):
        pivot = high.shift(1).rolling(window, min_periods=window).max()
        base_low = low.shift(1).rolling(window, min_periods=window).min()
        depth = 1.0 - base_low / pivot.replace(0, np.nan)
        prior_runup = close.shift(window + 1) / close.shift(window + 127) - 1.0
        first_20_volatility = returns.rolling(20, min_periods=20).std().shift(
            window - 19
        )
        handle_low = low.shift(1).rolling(10, min_periods=10).min()
        midpoint = base_low + (pivot - base_low) * 0.5
        window_valid = (
            prior_runup.ge(0.20)
            & depth.between(0.10, 0.35, inclusive="both")
            & volatility_20.le(first_20_volatility * 0.75)
            & handle_low.ge(midpoint)
            & close.gt(pivot)
            & close.le(pivot * 1.05)
        )
        structured = structured | window_valid.fillna(False)
    frame["structured_base_breakout"] = structured
    return frame
    ...
    for symbol, symbol_rows in selections.groupby("symbol", sort=True):
        ...
        for row in symbol_rows.itertuples(index=False):
            ...
            for label, signal_name in (
                ...
                ("structured-base-breakout", "structured_base_breakout"),
            ):
