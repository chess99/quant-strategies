...
ENTRY_MODE_DESCRIPTIONS = {
    ...
    "right-low-extension": (
        "使用完整双指标右侧入场条件，但反转 MA20/MA60 趋势间距排序，"
        "优先选择趋势延伸较小的信号；退出规则保持不变。"
    ),
}
...
def _right_low_extension_score(logic, frame):
    latest = frame.iloc[-1]
    ma20 = logic._finite_number(latest["ma20"])
    ma60 = logic._finite_number(latest["ma60"])
    trend_gap = ma20 / ma60 - 1.0 if np.isfinite(ma60) and ma60 > 0.0 else 0.20
    return 200.0 - min(max(trend_gap, 0.0), 0.20) * 100.0
...
def entry_signal_for_mode(logic, frame, mode):
    ...
    if mode == "right-low-extension":
        return (
            {
                "kind": "right",
                "score": _right_low_extension_score(logic, frame),
            }
            if logic.is_right_entry(frame)
            else None
        )
