...
def _validate_account(account: dict, expected_model: str | None = None) -> None:
    ...
    pending_order_id = account.get("pending_order_id")
    try:
        pending_shares = int(account.get("pending_shares", 0))
    except (TypeError, ValueError) as exc:
        raise OperationHalt("未决订单数量异常") from exc
    if bool(pending_order_id) != bool(pending_shares > 0):
        raise OperationHalt("未决订单标识与数量不一致")
...
def validate_snapshot(
    ...
    if require_cross_check and data.get("latest_session_cross_checked") is not True:
        raise OperationHalt("最新交易日尚未通过第二行情源核验")
...
def apply_fill(account: dict, order: dict, fill: dict) -> tuple[dict, dict]:
    ...
    if account.get("pending_order_id") not in {None, order["order_id"]}:
        raise OperationHalt("成交与账户未决订单不一致")
    side = fill.get("side")
    ...
    available_order_shares = (
        int(account["pending_shares"])
        if account.get("pending_order_id") == order["order_id"]
        else int(order["planned_shares"])
    )
    if shares <= 0 or shares % LOT_SIZE or shares > available_order_shares:
    ...
    if side == "buy":
        ...
        cash_after = cash_before + gross - fee
        shares_after = shares_before - shares
    remaining = available_order_shares - shares
    event = {
        "schema_version": 1,
...
def release_pending_order(account: dict, order_id: str, reason: str) -> tuple[dict, dict]:
    _validate_account(account)
    if not order_id or account.get("pending_order_id") != order_id:
        raise OperationHalt("待关闭订单与账户未决订单不一致")
    if reason not in {"cancelled", "expired", "rejected"}:
        raise OperationHalt("未决订单关闭原因不合法")
    updated = copy.deepcopy(account)
    event = {
        "schema_version": 1,
        "previous_hash": updated["last_event_hash"],
        ...
        "order_id": order_id,
        "reason": reason,
        "unfilled_shares": int(updated["pending_shares"]),
    }
    event["event_hash"] = _payload_hash(event)
    updated["pending_order_id"] = None
    updated["pending_shares"] = 0
    updated["rebalance_incomplete"] = True
    updated["last_event_hash"] = event["event_hash"]
    _validate_account(updated)
    return updated, event
...
def _account_map(value: dict) -> dict:
    accounts = value.get("accounts", value)
    if not isinstance(accounts, dict):
        raise OperationHalt("账户文件格式异常")
    return copy.deepcopy(accounts)


def _select_order(value: dict, order_id: str) -> dict:
    orders = value.get("orders") if isinstance(value, dict) else None
    candidates = orders if isinstance(orders, list) else [value]
    for order in candidates:
        if isinstance(order, dict) and order.get("order_id") == order_id:
            return order
    raise OperationHalt("订单文件中找不到成交对应订单")
...
def parse_args():
    ...
    apply = subparsers.add_parser("apply-fill", help="把一笔券商或模拟成交登记到账户")
    apply.add_argument("--accounts", type=Path, required=True)
    apply.add_argument("--plan", type=Path, required=True)
    apply.add_argument("--fill", type=Path, required=True)
    apply.add_argument("--output", type=Path)
    release = subparsers.add_parser("release-order", help="登记未成交部分的取消、过期或拒绝")
    release.add_argument("--accounts", type=Path, required=True)
    release.add_argument("--model", choices=FROZEN_CONFIG_NAMES, required=True)
    release.add_argument("--order-id", required=True)
    release.add_argument("--reason", choices=("cancelled", "expired", "rejected"), required=True)
    release.add_argument("--output", type=Path)
    reconcile = subparsers.add_parser("reconcile", help="对比理论账户与外部实际账户")
    reconcile.add_argument("--accounts", type=Path, required=True)
    reconcile.add_argument("--actual", type=Path, required=True)
    reconcile.add_argument("--model", choices=FROZEN_CONFIG_NAMES, required=True)
...
def main() -> int:
    ...
    try:
        if args.command == "initial":
        ...
        elif args.command == "apply-fill":
            account_map = _account_map(_read_json(args.accounts))
            plan = _read_json(args.plan)
            fill = _read_json(args.fill)
            order = _select_order(plan, str(fill.get("order_id") or ""))
            model_name = order["model"]
            updated, event = apply_fill(account_map[model_name], order, fill)
            account_map[model_name] = updated
            result = {
                "schema_version": 1,
                "status": "fill_applied",
        ...
        elif args.command == "release-order":
            account_map = _account_map(_read_json(args.accounts))
            updated, event = release_pending_order(
                account_map[args.model],
                args.order_id,
                args.reason,
            )
            account_map[args.model] = updated
            result = {
                "schema_version": 1,
            ...
            actual_map = actual.get("accounts", actual)
            reconciliation = reconcile_account(
                account_map[args.model],
                actual_map[args.model] if args.model in actual_map else actual_map,
            )
            result = {
                "schema_version": 1,
                "status": "reconciled",
