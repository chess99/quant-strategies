"""欧奈尔 CAN SLIM 策略族的本地 Qlib + 东方财富日线回测器。

平台策略源码仍是各变体的事实源。本文件只负责把本地 Qlib 行情、历史指数成分和
带公告日的财务缓存转换为策略纯函数需要的输入，并用次日开盘价近似聚宽 10:00 执行。
"""
...
FORMAL_CANDIDATE_PATH = STRATEGY_DIR / "variants" / "formal_candidate.py"
STRATEGY_PATHS = {
    "baseline": BASELINE_PATH,
    "formal-candidate": FORMAL_CANDIDATE_PATH,
}
COMMON_ENGINE_PATH = (
...
def load_strategy_logic(variant="baseline"):
    if variant not in STRATEGY_PATHS:
        raise ValueError("不支持的策略变体：%s" % variant)
    module_name = "oneil_canslim_joinquant_%s" % variant.replace("-", "_")
    return _load_module(module_name, STRATEGY_PATHS[variant])
...
def market_risk_budget(total_value, available_cash, exposure):
    total_value = float(total_value)
    available_cash = float(available_cash)
    exposure = min(max(float(exposure), 0.0), 1.0)
    current_gross = max(total_value - available_cash, 0.0)
    return max(total_value * exposure - current_gross, 0.0)
...
class BacktestConfig:
    ...
    market_symbols: tuple[str, ...] = ("SH000300", "SH000905")
    ...
    def __post_init__(self):
        ...
        self.market_symbols = tuple(self.market_symbols)
        ...
        if self.variant not in STRATEGY_PATHS:
            raise ValueError("不支持的策略变体：%s" % self.variant)
...
class LocalBacktester:
    def __init__(self, data, financials, config=None, logic=None, common=None):
        ...
        self.logic = (
            ...
            else load_strategy_logic(self.config.variant)
        )
    ...
    def _prepare_frames(self):
        ...
        if self.config.variant == "formal-candidate":
            symbols.update(self.config.market_symbols)
        self._log(
        ...
        self.scans.append(
            {
                ...
                "established_growth": int(
                    eligible.get("fundamental_track", pd.Series(dtype=str))
                    .eq("established_growth")
                    .sum()
                ),
                "emerging_leader": int(
                    eligible.get("fundamental_track", pd.Series(dtype=str))
    ...
    def _market(self, observation_date):
        if self.config.variant == "formal-candidate":
            ...
            for symbol in self.config.market_symbols:
                ...
                detail = self.logic.classify_market_regime(frame)
                details[symbol] = detail
                if detail["state"] != self.logic.MARKET_UNKNOWN:
                    states[symbol] = detail["state"]
            market = self.logic.aggregate_market_states(states)
            market["index_states"] = states
            market["index_details"] = details
            market["distribution_days"] = sum(
                int(detail.get("distribution_days", 0) or 0)
                for detail in details.values()
            )
            market["correction_reason"] = (
                "no_confirmed_index"
                if market["state"] == self.logic.MARKET_CORRECTION and states
                else None
            )
            market["transition_distribution_days"] = sum(
    ...
    def _exit_reason(self, symbol, observation_date, trade_date, market_state):
        ...
        if self.config.variant == "formal-candidate":
            arguments["close_20d_ma"] = ma20
        return self.logic.position_exit_reason(**arguments)
        ...
        for trade_date in trade_dates:
            ...
            market_exposure = float(
                market.get(
                    "exposure",
                    1.0
                    if market["state"] == self.logic.MARKET_CONFIRMED
                    else 0.0,
                )
            )
            if not exit_signals and market_exposure > 0.0:
                total_value = self._portfolio_value(trade_date, "open")
                risk_budget = market_risk_budget(
                    total_value,
                    self.cash,
                    market_exposure,
                )
                for symbol in list(self.positions):
                    ...
                    if next_stage > position.stage:
                        ...
                        additional = max(target_value - current_value, 0.0)
                        if additional > risk_budget:
                            continue
                        gross = self._buy(
                            symbol,
                            trade_date,
                        ...
                        gross_traded += gross
                        risk_budget = max(risk_budget - gross, 0.0)
...
def render_report(result, run_id):
    ...
    is_formal = result.config.variant == "formal-candidate"
    market_description = (
        "沪深300与中证500两指数投票代理；因本地缺少创业板指和科创50，最高新增风险预算为70%"
        if is_formal
        else "沪深300代理聚宽基线的上证综指"
    )
    candidate_description = (
        "成熟增长/景气反转双通道；本地缓存没有业绩预告，反转通道只能使用已公告财报的利润改善"
        if is_formal
        else "严格成长基线"
    )
...
- 候选口径：{candidate_description}。
- 市场状态：{market_description}。
...
def archive_result(result, run_id, archived_at=None):
    ...
    target = (
        STRATEGY_DIR
        / "backtests"
        / f"{archived_at:%Y-%m-%d}__{result.config.variant}__{run_id}"
    )
    if target.exists():
        raise FileExistsError(f"回测归档已存在，禁止覆盖：{target}")
    raw_dir = target / "raw"
    raw_dir.mkdir(parents=True)
    source_path = STRATEGY_PATHS[result.config.variant]
    ...
    manifest = {
        ...
        "engine": "local-qlib-eastmoney-daily-v2",
        "run_id": run_id,
        ...
        "limitations": [
            ...
            "local index coverage proxies the JoinQuant market regime",
            "local financial cache does not include point-in-time earnings forecasts",
