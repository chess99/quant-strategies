...
迭代 2 的事实源由上交所、深交所、巨潮、新浪、同花顺和东方财富交叉构建。候选池不是
“今天仍上市的 ETF”倒推历史：

- 上交所使用 2012 年以来的逐月官方规模快照恢复历史产品；
- 巨潮基金公告按四年分块完整分页，检索 2005 年至观察日的“终止上市”公告，恢复
  沪深两市历史终止产品；
...
2026-07-27 全量验收得到 1,781 只候选、1,733 只有历史日线，共
1,526,502 行，日期覆盖 2005-02-23 至 2026-07-24。当前有效目标 1,618 只，
其中 1,608 只有日线，覆盖率 99.38%。另恢复 125 只已终止 ETF 的交易区间，
其中上交所 79 只、深交所 46 只；巨潮识别出的 46 只真实终止上市候选全部取得日线。
...
`raw/sina/`、`raw/eastmoney/`、`raw/sse/` 与 `raw/cninfo/`，manifest 保存逐文件
SHA-256。同一观察日的上游快照若内容改变，同步器以内容哈希另存新版本，不覆盖旧 raw。
...
- 当前 10 只 `active_no_history` 产品已由当前清单和基金档案确认，但在行情出现前不能
  交易。
- 上交所历史规模快照从 2012 年开始；更早历史由 2005 年以来的巨潮终止上市公告与
  新浪逐只行情补证。
- 深交所没有同等公开的逐月历史快照；历史池依赖巨潮完整公告分页和逐只行情交叉恢复，
  因此成功行情仍标 B 而非权威 A 级。

策略必须按 `listing_date`、`delisting_date` 和观察日期构造 ETF 池。只用当前有效池
回测历史会遗漏至少 125 只已恢复的终止产品，产生幸存者偏差。

## 黄金对照

完整归档位于
`studies/joinquant-etf-rotation-replication/results/2026-07-27__local-full-etf-universe-v4/`。
