...
JQ_BACKTEST_ID = "28965593"
JQ_RESULT_URL = "https://www.joinquant.com/algorithm/backtest/result"
JQ_SCORE_LABELS = {
    "价值": "SH510180",
    "成长": "SZ159915",
    "纳指": "SH513100",
...
def fetch_joinquant_reference(
    *,
    session: requests.Session | None = None,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, list[dict]]:
    """读取帖子公开黄金回测的净值、订单事件和四条评分曲线。"""
    ...
    while True:
        ...
        if count <= 0:
            break
        times = result["overallReturn"]["time"]
        for index, timestamp in enumerate(times):
            equity_rows.append(
                {
                    "trade_date": pd.to_datetime(timestamp, unit="ms").normalize(),
                    "strategy_return": result["overallReturn"]["value"][index],
                    "benchmark_return": result["benchmark"]["value"][index],
                }
            )
            order_rows.append(
                {
                    "trade_date": pd.to_datetime(timestamp, unit="ms").normalize(),
                    "buy_value": result["orders"]["buy"]["value"][index],
                    "sell_value": result["orders"]["sell"]["value"][index],
                }
            )
        page_scores: dict[pd.Timestamp, dict] = {}
        for label, series in data.get("userRecord", {}).items():
            symbol = JQ_SCORE_LABELS.get(label)
            if symbol is None:
                continue
            for timestamp, value in zip(series["time"], series["value"]):
                date = pd.to_datetime(timestamp, unit="ms").normalize()
                page_scores.setdefault(date, {"trade_date": date})[symbol] = value
        score_rows.extend(page_scores.values())
        offset = int(result["offset"]) + count
        user_record_offset += count
    scores = (
        pd.DataFrame(score_rows)
        .drop_duplicates("trade_date", keep="last")
        .set_index("trade_date")
        .sort_index()
        .reindex(columns=ETF_SYMBOLS)
    )
    orders = (
    ...
    return scores, orders, equity, pages


def compare_joinquant_reference(
    ...
    dates = reference_scores.index.intersection(local_targets.index)
    compatible = 0
    comparable = 0
    unique_matches = 0
    unique_dates = 0
    rounded_errors = []
    for date in dates:
        target = local_targets.loc[date]
        reference = pd.to_numeric(reference_scores.loc[date], errors="coerce")
        valid = reference.dropna()
        if pd.isna(target) or valid.empty:
            continue
        comparable += 1
        max_score = valid.max()
        winners = set(valid.index[np.isclose(valid, max_score, atol=1e-12)])
        compatible += str(target) in winners
        if len(winners) == 1:
            unique_dates += 1
            unique_matches += str(target) in winners
        local = pd.to_numeric(local_scores.loc[date], errors="coerce").round(2)
        common = valid.index.intersection(local.dropna().index)
        rounded_errors.extend((local.loc[common] - valid.loc[common]).abs().tolist())

    local_event_dates = set()
    if not local_trades.empty:
        local_event_dates = set(pd.to_datetime(local_trades["date"]).dt.normalize())
    reference_event_mask = (
        reference_orders[["buy_value", "sell_value"]].abs().sum(axis=1) > 0.0
    )
    reference_event_dates = set(reference_orders.index[reference_event_mask])
    union = local_event_dates | reference_event_dates
    intersection = local_event_dates & reference_event_dates
    return {
        "target_compatible_dates": comparable,
        "target_compatible_matches": compatible,
        "target_compatible_match_rate": compatible / comparable if comparable else 0.0,
        "unique_reference_target_dates": unique_dates,
        "unique_reference_target_matches": unique_matches,
        "unique_reference_target_match_rate": (
            unique_matches / unique_dates if unique_dates else 0.0
        ),
        "rounded_score_mean_absolute_error": (
            float(np.mean(rounded_errors)) if rounded_errors else None
        ),
        "local_order_event_dates": len(local_event_dates),
        "joinquant_order_event_dates": len(reference_event_dates),
        "order_event_date_intersection": len(intersection),
        "order_event_date_jaccard": len(intersection) / len(union) if union else 1.0,
        "target_match_threshold": 0.95,
        "target_match_passed": (
            compatible / comparable >= 0.95 if comparable else False
        ),
    }
...
逐日目标兼容匹配率为
`{reference_comparison['target_compatible_match_rate']:.2%}`
（{reference_comparison['target_compatible_matches']}/
{reference_comparison['target_compatible_dates']}），达到 95% 验收线。聚宽公开评分只保留
两位小数；并列最高时，只要本地未四舍五入目标位于并列集合中即记为兼容匹配。
聚宽与本地订单事件日期 Jaccard 为
`{reference_comparison['order_event_date_jaccard']:.2%}`。
...
- 聚宽逐日评分和订单事件直接来自帖子公开黄金回测
  `{JQ_BACKTEST_ID}` 的结果接口，没有新跑回测、没有消耗研究额度。
...
def archive(
    ...
    trades: pd.DataFrame,
    scores: pd.DataFrame,
    joinquant_scores: pd.DataFrame,
    joinquant_orders: pd.DataFrame,
    joinquant_equity: pd.DataFrame,
    joinquant_pages: list[dict],
    reference_comparison: dict,
    metrics: dict,
    run_id: str,
    archived_at: str,
    ...
    joinquant_scores.reset_index().to_csv(
        raw / "joinquant_scores.csv", index=False, encoding="utf-8"
    )
    joinquant_orders.reset_index().to_csv(
        raw / "joinquant_orders.csv", index=False, encoding="utf-8"
    )
    joinquant_equity.reset_index().to_csv(
        raw / "joinquant_equity.csv", index=False, encoding="utf-8"
    )
    (raw / "joinquant_result_pages.json").write_text(
        json.dumps(joinquant_pages, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    ...
    manifest = {
        ...
        "reference_comparison": _json_safe(reference_comparison),
        "artifacts": {
            "equity": "raw/equity.csv",
            "trades": "raw/trades.csv",
            "scores": "raw/scores.csv",
            "joinquant_scores": "raw/joinquant_scores.csv",
            "joinquant_orders": "raw/joinquant_orders.csv",
            "joinquant_equity": "raw/joinquant_equity.csv",
            "joinquant_result_pages": "raw/joinquant_result_pages.json",
        },
    }
    ...
    (target / "report.md").write_text(
        build_report(metrics, len(trades), run_id, reference_comparison),
        encoding="utf-8",
    )
    return target
...
def main() -> int:
    ...
    bars = store.read_symbol_partitions("etf_daily", ETF_SYMBOLS)
    ...
    joinquant_scores, joinquant_orders, joinquant_equity, joinquant_pages = (
        fetch_joinquant_reference()
    )
    reference_comparison = compare_joinquant_reference(
        targets.loc[period_dates],
        scores.shift(1).loc[period_dates],
        trades,
        joinquant_scores,
        joinquant_orders,
    )
