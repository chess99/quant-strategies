...
def load_or_build_monthly_snapshots(store, observation_dates, execution_dates):
    ...
    return valuation_result.frame, state_result.frame, names, scans
...
def build_targets(store: ResearchDataStore, schedule: pd.DataFrame):
    ...
    valuation, state, historical_names, scan_audits = load_or_build_monthly_snapshots(
    ...
    for row in schedule.itertuples(index=False):
        ...
        cross["eligible"] = (
            ...
            & ~cross["historical_display_name"].fillna("").str.contains("退")
        )
