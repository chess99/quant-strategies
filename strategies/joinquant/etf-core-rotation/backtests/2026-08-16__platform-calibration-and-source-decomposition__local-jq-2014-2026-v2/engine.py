...
def weekly_execution_pairs(
    ...
    for observation in week_last.iloc[:-1]:
        ...
        if len(future) >= lag_sessions:
            pairs.append(
                (pd.Timestamp(observation), pd.Timestamp(future[lag_sessions - 1]))
            )
    return pairs
...
def compose_target_weights(
    ...
    """Compose risk and defensive sleeves without changing frozen baseline defaults."""

    if allocation_mode == "defensive_only":
        return {choose_defensive_asset(observation_date, data): 1.0}
    target_weights = dict(risk_weights)
    if allocation_mode == "risk_cash":
        return target_weights
    if allocation_mode == "risk_normalized":
        total = sum(target_weights.values())
        if total > 1e-12:
            return {symbol: weight / total for symbol, weight in target_weights.items()}
        return {}
...
def run_simulation_fast(
    ...
    observation_by_execution = {
        ...
        for observation, execution in weekly_execution_pairs(
            data.trade_dates, config.execution_lag_sessions
        )
    ...
    for execution_index, execution_date in enumerate(execution_dates):
        ...
        target_weights = compose_target_weights(
            risk_weights,
            observation_date,
            data,
